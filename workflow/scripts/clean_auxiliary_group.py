...
def main(snakemake: Any) -> None:
    ...
    if not batches:
        raise ValueError(
            ...
            "contains no source batches."
        )

    starts = {
        pd.Timestamp(batch["start"])
        for batch in batches
    }

    ends = {
        pd.Timestamp(batch["end"])
        for batch in batches
    }
    ...
    if len(starts) != 1 or len(ends) != 1:
        raise ValueError(
            f"Auxiliary group {group_id!r} "
            "contains inconsistent batch periods."
        )
