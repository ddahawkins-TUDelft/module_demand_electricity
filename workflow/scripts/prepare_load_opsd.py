...
def main(path_raw_load, output_load, start, end, country_codes):
    ...
    load = load.loc[
        (load["utc_timestamp"] >= start) & (load["utc_timestamp"] < end)
    ].copy()
    ...
    prepared = prepared.reindex(index=target_index, columns=country_codes)
...
if __name__ == "__main__":
    ...
    if plan_path is not None:
        ...
        batch = get_batch(
            plan, batch_id=snakemake.wildcards.batch_id, source="opsd_api"
        )
        start = batch["start"]
        end = batch["end"]
