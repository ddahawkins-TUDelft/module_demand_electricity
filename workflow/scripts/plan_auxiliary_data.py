...
def build_advanced_execution_plan(
    *,
    ...
    advanced_rules = build_advanced_rules(
        gap_filling_config
    )
    ...
    active_source_names = _get_active_source_names(
        active_rules
    )

    constructed_source_periods = (
        build_all_constructed_source_periods(
            gap_filling_config,
    ...
    requirements = (
        build_auxiliary_acquisition_requirements(
            list(
                constructed_source_periods.values()
            ),
    ...
    raw_batches = build_source_batches(
        requests
    )
...
def _build_rule_manifest(
    active_rules: pd.DataFrame,
    *,
    source_definitions: Mapping[
        str,
        Mapping[str, Any],
    ],
    constructed_source_periods: Mapping[
        str,
        pd.DataFrame,
    ],
    ...
    for rule in active_rules.itertuples(
        ...
        if (
            ...
            if (
                source_name
                not in constructed_source_periods
            ):
                raise ValueError(
    ...
    for rule in active_rules.itertuples(
        ...
        if pd.isna(rule.source):
            raise ValueError(
                ...
                f"{rule.rule_name!r} has no source."
            )
