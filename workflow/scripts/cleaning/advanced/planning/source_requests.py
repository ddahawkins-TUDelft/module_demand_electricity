...
def build_auxiliary_source_requests(
    ...
    for source_name in source_names:
        for row in requirements.itertuples(index=False):
            if not _source_supports_country(source_name, row.country):
                continue
    ...
    return pd.DataFrame(requests, columns=SOURCE_REQUEST_COLUMNS)


def _source_supports_country(source_name: str, country: str) -> bool:
    """Return whether a source is structurally applicable to a country."""
    if source_name == "neso":
        return country == "GBR"

    if source_name in {"entsoe_api", "opsd_api"}:
        return True
        ...
        batches.append(
            {
                ...
                "batch_id": _build_batch_id(
                    source=source, start=start, end=end, countries=countries
                ),
                "source": source,
                "start": start,
                "end": end,
...
def _build_batch_id(
    *, source: str, start: pd.Timestamp, end: pd.Timestamp, countries: list[str]
) -> str:
...
def _build_group_id(*, start: pd.Timestamp, end: pd.Timestamp) -> str:
