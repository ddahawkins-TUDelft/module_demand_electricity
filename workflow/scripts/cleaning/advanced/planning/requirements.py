...
def compile_auxiliary_requirements(
    ...
    for rule in overrides.values():
        ...
        requirements.extend(_collect_sources(rule["sources"]))

        scaling = rule.get("scaling")

        if scaling is not None:
            requirements.extend(_collect_sources(scaling["target_sources"]))

    if not requirements:
        return pd.DataFrame(columns=REQUIREMENT_COLUMNS)
    ...
    return _merge_requirements(requirements_frame)


def _collect_sources(sources: list[Mapping[str, Any]]) -> list[dict[str, Any]]:
...
def get_basic_cleaning_context(
    ...
    for rule in rules:
        ...
        if method == "linear_interpolation":
            ...
            offsets = [pd.Timedelta(offset) for offset in rule["source_offsets"]]
        ...
        left = min(previous_left, rule_left)
        right = max(previous_right, rule_right)
...
def build_auxiliary_acquisition_requirements(
    *,
    ...
    exact_requirements = compile_auxiliary_requirements(overrides)
