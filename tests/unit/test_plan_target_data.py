...
def test_effective_scope_returns_none_without_overlap(
    source_start,
    source_end,
):
...
def test_missing_or_empty_contexts_means_no_geographic_restriction(
    metadata,
):
...
def test_uncovered_intervals_reports_internal_gap():
    ...
    assert gaps == [
        (
            ...
            pd.Timestamp("2019-02-01T00:00:00Z"),
        )
    ]
...
def test_target_plan_accepts_contiguous_source_coverage():
    ...
    assert plan["active_sources"] == [
        "source_a",
        "source_b",
    ]

    assert plan["source_contexts"] == {
        "source_a": ["AAA"],
        "source_b": ["AAA"],
    }
...
def test_target_plan_excludes_temporally_inactive_source():
    ...
    assert (
        plan["source_temporal_scopes"]["inactive_source"]
        is None
    )
...
def test_target_plan_sorts_and_deduplicates_target_contexts():
    ...
    assert plan["target_contexts"] == [
        "AAA",
        "BBB",
        "CCC",
    ]

    assert plan["source_contexts"]["source_a"] == [
        "AAA",
        "BBB",
        "CCC",
    ]
...
def test_target_plan_rejects_unknown_source():
    ...
    with pytest.raises(
        ValueError,
        match="Unsupported electricity-demand source",
    ):
        build_target_data_plan(
            target_contexts=["AAA"],
...
def test_target_plan_rejects_empty_target_contexts():
    ...
    with pytest.raises(
        ValueError,
        match="no land-country contexts",
    ):
        build_target_data_plan(
            target_contexts=[],
            source_names=["source_a"],
